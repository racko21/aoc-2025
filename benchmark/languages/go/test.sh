...
# which performs read+parse+solve for that part — matching the C arm's and the
# retroactively-instrumented Go arm's per-part timing convention.
set -u
WORK="$1"
PART="$2"
BIN="$WORK/main"
...
START_NS=$(date +%s%N)
if [ -n "$TIME_BIN" ]; then
  $TIME_BIN ./main "$PART" > /tmp/go_sol_out 2> /tmp/go_sol_time
  RC=$?
else
  ./main "$PART" > /tmp/go_sol_out 2> /tmp/go_sol_err
  RC=$?
fi
END_NS=$(date +%s%N)
...
PEAK_KB=""
if [ -n "$TIME_BIN" ] && [ -f /tmp/go_sol_time ]; then
  PEAK_KB=$(grep "Maximum resident set size" /tmp/go_sol_time | grep -oE '[0-9]+' | head -1)
fi

grep -E "^Part[[:space:]]*${PART}[[:space:]]*:" /tmp/go_sol_out || true
